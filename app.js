...
function load() {
  ...
  return {
    ...
    completed: {},
    saves: {},
    stats: { totalCompleted: 0, streak: 0, longest: 0, hints: 0 },
...
function syncLabel() {
  if (!state.settings.cloudSync) return "Cloud off";
  if (!supabase) return "Local only";
  if (!state.cloud.userId) return "Local save active";
...
function gamesView() {
  ...
  return `<section class="stack"><article class="panel"><div class="route-head"><h2 class="section-title">${meta[state.game].title}</h2><span class="muted">${syncLabel()}</span></div><div class="difficulty-row">${DIFFICULTIES.map((d) => `<button class="difficulty-pill ${d === state.difficulty ? "active" : ""}" data-action="difficulty" data-difficulty="${d}" type="button">${cap(d)}</button>`).join("")}</div></article><article class="panel"><div class="level-grid">${levels.map((level) => { const done = state.completed[key(state.game, state.difficulty, level)]; return `<button class="level-tile ${done ? "done" : ""}" data-action="open" data-game="${state.game}" data-difficulty="${state.difficulty}" data-level="${level}" type="button">${done ? "✓" : String(level).padStart(3, "0")}</button>`; }).join("")}</div></article></section>`;
}
...
function boardView(save, def) {
  if (save.game === "sudoku") {
    return `<div class="sudoku-board">${def.puzzle.map((row, ri) => row.map((given, ci) => { const v = save.board[ri][ci]; return `<button class="cell ${given ? "given" : ""} ${ci % 3 === 2 && ci < 8 ? "block-right" : ""} ${ri % 3 === 2 && ri < 8 ? "block-bottom" : ""}" data-action="cell" data-row="${ri}" data-col="${ci}" type="button">${v ? `<span>${v}</span>` : noteGrid(save.notes[ri][ci] || [])}</button>`; }).join("")).join("")}</div>`;
  }
  if (save.game === "kakuro") {
    ...
    for (let r = 0; r < size; r += 1) for (let c = 0; c < size; c += 1) {
      ...
      else { const given = def.puzzle[r - 1][c - 1]; const v = save.board[r - 1][c - 1]; cells.push(`<button class="kakuro-cell ${given ? "given" : ""}" data-action="cell" data-row="${r - 1}" data-col="${c - 1}" type="button">${v ? `<span>${v}</span>` : noteGrid(save.notes[r - 1][c - 1] || [])}</button>`); }
    }
  ...
  for (let r = 0; r < size; r += 1) for (let c = 0; c < size; c += 1) {
    ...
    else { const v = def.puzzle[r - 1][c - 1]; const mode = save.board[r - 1][c - 1]; cells.push(`<button class="sumplete-cell ${mode === "remove" ? "removed" : ""} ${mode === "lock" ? "locked" : ""}" data-action="cell" data-row="${r - 1}" data-col="${c - 1}" type="button">${v}</button>`); }
  }
...
function playView() {
  ...
  return `<section class="stack">${save.completed ? `<article class="celebration panel"><p class="eyebrow">Puzzle Complete</p><h2>${meta[save.game].title} cleared</h2><p class="muted">Time ${fmt(save.timer)} • Progress saved.</p></article>` : ""}<article class="board-shell"><div class="route-head"><div><p class="eyebrow">${meta[save.game].title}</p><h2 class="board-title">Level ${save.level}</h2></div><div class="board-summary"><strong>${fmt(save.timer)}</strong><span class="muted">${syncLabel()}</span></div></div><div class="board-frame">${boardView(save, def)}</div><div class="board-actions"><div class="control-strip"><button class="toggle-button ${state.noteMode ? "active" : ""}" data-action="notes" type="button">Notes</button><button class="toggle-button" data-action="hint" type="button">Hint</button></div>${pad}</div></article></section>`;
}

function progressView() {
  return `<section class="stack"><article class="panel"><div class="section-head"><h2 class="section-title">Overall Progress</h2><strong>${state.stats.totalCompleted} solved</strong></div><p class="muted">Long journey mode. Every game scales to 10,000 seeded levels.</p>${GAMES.map((game) => `<div class="chart-row"><div class="stats-line"><span>${meta[game].title}</span><strong>${completedCount(game)} / ${LEVELS}</strong></div><div class="bar-track"><div class="bar-fill" style="width:${pct(game)}%"></div></div></div>`).join("")}</article></section>`;
}
...
function handleAction(event) {
  ...
  if (action === "library") { state.game = btn.dataset.game; state.route = "games"; render(); return; }
  if (action === "difficulty") { state.difficulty = btn.dataset.difficulty; persistNow(false); render(); return; }
  if (action === "start") {
  ...
  if (action === "open") {
    const save = ensureSave(btn.dataset.game, btn.dataset.difficulty, Number(btn.dataset.level));
    state.currentKey = save.key; state.route = "play"; upsertRecent(save.key); persist(); render(); return;
  }
  if (action === "resume") { state.currentKey = btn.dataset.key; state.route = "play"; upsertRecent(btn.dataset.key); persist(); render(); return; }
  if (action === "cell") { const save = currentSave(); if (save && save.game === "sumplete") { const row = Number(btn.dataset.row); const col = Number(btn.dataset.col); const current = save.board[row][col]; const next = current === "keep" ? "remove" : current === "remove" ? "lock" : "keep"; save.board[row][col] = next; markTouched(save); solveCheck(save); render(); return; } state.selected = { row: Number(btn.dataset.row), col: Number(btn.dataset.col) }; return; }
  ...
  if (action === "erase") { const save = currentSave(); if (!save || !state.selected) return; const { row, col } = state.selected; save.board[row][col] = 0; save.notes[row][col] = []; markTouched(save); render(); return; }
...
setInterval(() => {
  ...
  save.updatedAt = new Date().toISOString();
  persist();
  const node = document.querySelector(".board-summary strong");
