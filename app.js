...
function playView() {
  ...
  return `<section class="stack">${save.completed ? `<article class="celebration panel"><p class="eyebrow">Puzzle Complete</p><h2>${meta[save.game].title} cleared</h2><p class="muted">Time ${fmt(save.timer)} • ${isBest ? "New personal best" : "Progress saved"}.</p><div class="quick-grid"><article class="stat-card"><span class="muted">Completed</span><strong>${gameStats.completed} / ${LEVELS}</strong></article><article class="stat-card"><span class="muted">Best Time</span><strong>${bestForGame ? fmt(bestForGame) : fmt(save.timer)}</strong></article></div><div class="action-row"><button class="accent-button" data-action="next-level" type="button">Next Level</button><button class="ghost-button" data-action="replay-level" type="button">Replay</button><button class="ghost-button" data-action="back-games" type="button">Back to Menu</button></div></article>` : ""}<article class="board-shell"><div class="route-head"><div><p class="eyebrow">${meta[save.game].title}</p><h2 class="board-title">Level ${save.level}</h2></div><div class="board-summary"><strong>${fmt(save.timer)}</strong><span class="muted">${syncLabel()}</span></div></div>${save.game === "kakuro" ? `<article class="list-card"><h3>Cross Sum Mode</h3><p class="muted">Fill digits so each row and column matches clue totals. Notes help for planning.</p></article>` : ""}<div class="board-frame">${boardView(save, def)}</div><div class="board-actions"><div class="control-strip"><button class="toggle-button ${state.noteMode ? "active" : ""}" data-action="notes" type="button">Notes</button><button class="toggle-button" data-action="hint" type="button">Hint</button><button class="toggle-button" data-action="undo" type="button">Undo</button><button class="toggle-button" data-action="redo" type="button">Redo</button></div>${pad}</div></article></section>`;
}
