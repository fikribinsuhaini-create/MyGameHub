...
function boardView(save, def) {
  ...
  if (save.game === "kakuro") {
    ...
    for (let r = 0; r < size; r += 1) for (let c = 0; c < size; c += 1) {
      ...
      else { const given = def.puzzle[r - 1][c - 1]; const v = save.board[r - 1][c - 1]; const selected = state.selected && state.selected.row === r - 1 && state.selected.col === c - 1; const invalid = !given && v && v !== def.solution[r - 1][c - 1]; cells.push(`<button class="kakuro-cell ${given ? "given" : ""} ${selected ? "selected" : ""} ${invalid ? "invalid" : ""}" data-action="cell" data-row="${r - 1}" data-col="${c - 1}" type="button">${v ? `<span>${v}</span>` : noteGrid(save.notes[r - 1][c - 1] || [])}</button>`); }
    }
